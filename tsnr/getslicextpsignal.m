...
function [] = getslicextpsignal(subj, runsvec, masktype, exp_name)%getslicextpsignal(subType,dataType,subj,masktype)
...
par.subdir =fullfile('/Users/thackery/Work/Circmaze/bobspiketest/', subj)%fullfile('/mnt/wagner/thackery/', exp_name, '/subs_preprocessed/', subj);
...
for idxr = 1:length(runfolds)
    allrawfilenames{idxr,1} = dir(fullfile(par.funcdir, runfolds(idxr).name, '/59*.nii'));%'/run*_4d.nii'));
...
%inplane=fullfile('/Users/vacarr/Desktop/fMRI_MCI',subType,subj,'raw/niftis_undistorted','mean.nii'); % mean inplane scan -- used to generate relevant brain masks
...
for s=1:length(scans)
    ...
    for tp=1:size(V,4)
        ...
        V(:,:,:,tp)=tmp;
        tmp=tmp-nanmean(tmp(:)); %take out global signal
        Vc(:,:,:,tp)=tmp;
